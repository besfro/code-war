...
// 冒泡
function bubbleSrot(arr, compareFn = Compare.defaultCompare) {
  let i = arr.length - 1
  while(i >= 0) {
    let j = i - 1
    while(j >= 0) {
      if(
        Compare.runCompare( compareFn, arr[j], arr[i] ).isSmall()
      ) {
...
runNext(
  [1000],
  count => console.log(
    bubbleSrot(createArray(count), (a, b) => a - b)
  )
)
...
runNext(
  [1000],
  count => console.log(
    bubbleSrot(createArray(count, 'object'), (a, b) => a.count - b.count)
  )
)
